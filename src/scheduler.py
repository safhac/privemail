...
async def async_email_fetch_job() -> bool:
    ...
    try:
        ...
        if 'no-reply' in email_address.lower() or 'noreply' in email_address.lower():
            logging.info(f"SCHEDULER: Archiving no-reply from {email_address}.")
            email_obj = Email(
        ...
        if not contact:
            logging.info(f"SCHEDULER: Creating new contact: {real_name or email_address}")
            contact = Contact(
        ...
        email_obj.correspondent_tone = analysis_result.get('correspondent_tone')
        email_obj.correspondent_goal = analysis_result.get('correspondent_goal')
        email_obj.correspondent_evidence = analysis_result.get('correspondent_evidence')
        db.commit()
        ...
        if manual_mode:
            logging.info(f"SCHEDULER: Manual Mode ON. Creating pending draft for {email_id}.")
            new_draft = Draft(
                email_id=email_id,
...
async def run_scheduler_loop(default_interval: int):
    logging.info(f"SCHEDULER: Loop starting. Default Interval: {default_interval}s")
    while True:
        ...
        if success:
            ...
            try:
                ...
                wait_time = int(interval_str) if interval_str else default_interval
            except:
            ...
            wait_time = 60
            logging.warning("SCHEDULER: Job failed. Entering short backoff (60s).")
