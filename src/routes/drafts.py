...
def get_draft_detail(draft_id: int, db: Session = Depends(get_db)):
    try:
        ...
        if not draft.email:
            raise HTTPException(status_code=404, detail="Draft has no associated email")
...
async def confirm_draft(draft_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to confirm draft: {e}")
...
async def send_draft(
    ...
    draft = db.query(Draft).options(joinedload(Draft.email)).filter(Draft.id == draft_id).first()
    if not draft:
        raise HTTPException(status_code=404, detail="Draft not found")
    
    if not draft.email:
        raise HTTPException(status_code=400, detail="Draft is not linked to an email context.")

    try:
        # Check Signature Setting
        include_sig = _get_setting(db, "include_signature")
        should_add_sig = True if include_sig is None else (include_sig == "true")
        ...
        if mode == "send":
            ...
            success = google_client.send_reply(service, to_email, subject, final_content)
            action_msg = "Email sent successfully."
        else:
            logging.info(f"Creating GMAIL DRAFT for {draft_id}...")
            success = google_client.create_draft(service, to_email, subject, final_content)
            action_msg = "Draft created in Gmail."
        
        if not success:
            raise HTTPException(status_code=500, detail="Failed to communicate with Gmail API.")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process draft: {e}")
...
async def generate_draft_manually(draft_id: int, db: Session = Depends(get_db)):
    ...
    contact = db.query(Contact).filter(Contact.email_address == sender_email_str).first()
    
    # Fetch contact or create temporary one for context
    contact_obj = contact if contact else Contact(email_address=sender_email_str, auto_draft_enabled=True)
...
async def rewrite_draft_paragraph(
    ...
    # 2. Get the Context
    original_context = draft.email.body_text if (draft.email and draft.email.body_text) else "No context available."
